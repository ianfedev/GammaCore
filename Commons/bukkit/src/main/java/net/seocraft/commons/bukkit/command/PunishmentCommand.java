...
public class PunishmentCommand implements CommandClass {
    ...
    @Inject
    private OnlineStatusManager onlinePlayers;
    @Inject
    ...
    public boolean banCommand(@Injected(true) @Sender Player player, OfflinePlayer target, @Optional("") String duration, @Optional("") @ConsumedArgs(-1) String reason, @Flag('s') boolean silent) {
    ...
    public boolean kickCommand(@Injected(true) @Sender Player player, OfflinePlayer target, @Optional("") @ConsumedArgs(-1) String reason, @Flag('s') boolean silent) {
        CallbackWrapper.addCallback(this.userStorageProvider.getCachedUser(player.getDatabaseIdentifier()), userAsyncResponse -> {
            if (userAsyncResponse.getStatus() == AsyncResponse.Status.SUCCESS) {
                User user = userAsyncResponse.getResponse();
    ...
    public boolean warnCommand(@Injected(true) @Sender Player player, OfflinePlayer target, @Optional("") @ConsumedArgs(-1) String reason, @Flag('s') boolean silent) {
        CallbackWrapper.addCallback(this.userStorageProvider.getCachedUser(player.getDatabaseIdentifier()), userAsyncResponse -> {
            if (userAsyncResponse.getStatus() == AsyncResponse.Status.SUCCESS) {
                User user = userAsyncResponse.getResponse();
